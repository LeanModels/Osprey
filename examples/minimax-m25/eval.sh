...
#
#   bash examples/minimax-m25/eval.sh <DRAFT_CHECKPOINT> [SUITE]
#
...
#   public       paper Table 2 - humaneval, math500, livecodebench, mtbench,
#                commonsense_eval at 64 prompts each
#   multilingual paper Table 3 - mgsm (64), global_mmlu (84),
...
if [ $# -lt 1 ]; then
    echo "usage: bash examples/minimax-m25/eval.sh <DRAFT_CHECKPOINT> [public|multilingual|all]" >&2
    exit 2
fi
...
# Default result name: <arm>-<train-domain>-<suite> for a Stage-4 directory
# .../<arm>/<train-domain>/epoch_*_step_*, otherwise <basename>-<suite>.
if [ -z "${RESULT_NAME:-}" ]; then
    case "$(basename "$DRAFT_CKPT")" in
        epoch_*) RESULT_NAME="$(basename "$(dirname "$(dirname "$DRAFT_CKPT")")")-$(basename "$(dirname "$DRAFT_CKPT")")-$SUITE" ;;
        *)       RESULT_NAME="$(basename "$DRAFT_CKPT")-$SUITE" ;;
    esac
fi
