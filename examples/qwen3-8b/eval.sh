...
#
#   bash examples/qwen3-8b/eval.sh <CHECKPOINT_DIR>
#
...
if [ $# -lt 1 ]; then
    echo "usage: bash examples/qwen3-8b/eval.sh <CHECKPOINT_DIR>" >&2
    exit 2
fi
...
# Default result name. A Stage-4 directory .../<arm>/<train-domain>/epoch_*_step_*
# gives <arm>-<train-domain>; any other directory, such as a released checkpoint
# osprey/qwen3-8b/osprey-math, gives its own basename.
if [ -z "${RESULT_NAME:-}" ]; then
    case "$(basename "$DRAFT_CKPT")" in
        epoch_*)
            ...
            ;;
        *)
            RESULT_NAME=$(basename "$DRAFT_CKPT")
            ;;
    esac
fi
