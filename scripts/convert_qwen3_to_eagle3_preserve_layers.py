#!/usr/bin/env python3
"""Convert a shallow Qwen3 checkpoint into the Osprey EAGLE-3 format.
...
def convert_state(
    ...
    for layer_idx in range(num_layers):
        ...
        # Copied verbatim from source. The eagle3 attention applies these
        # AFTER projection and reshape to per-head; the (head_dim,) weight
        # is invariant to the 2x QKV input expansion done above.
        out[f"{dst}.self_attn.q_norm.weight"] = source_state[
            f"{src}.self_attn.q_norm.weight"
        ].contiguous()
        out[f"{dst}.self_attn.k_norm.weight"] = source_state[
            f"{src}.self_attn.k_norm.weight"
        ].contiguous()
...
def make_eagle_config(
    ...
    eagle["architectures"] = ["Qwen3ForCausalLMEagle3"]
