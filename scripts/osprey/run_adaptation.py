...
TARGETS: Dict[str, TargetPreset] = {
    ...
    # Table 1 / Appendix E: bf16 Llama-3.3-70B-Instruct at tp=4 on one 8-GPU
    # node. 100k samples x 3 epochs / 8 GPUs = 37,500 steps.
    "llama33-70b": TargetPreset(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Stop after this many optimizer steps. The learning-rate schedule "
        "still spans --num-epochs, so this reproduces a run that was stopped "
        "early rather than one scheduled shorter. The paper's MiniMax-M2.5 "
        "adaptation stops at 35000.",
    )
...
def build_command(args: argparse.Namespace) -> List[str]:
    ...
    if max_num_steps is not None:
        command += ["--max-num-steps", str(max_num_steps)]
