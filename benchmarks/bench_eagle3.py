...
def parse_args():
    ...
    benchmark_group.add_argument(
        "--domain-eval-data-dir",
        type=str,
        default=None,
        help="Directory containing the five Osprey domain evaluation JSONL files.",
    )
    benchmark_group.add_argument(
        "--domain-eval-suffix",
        type=str,
        default="_4096",
        help="Filename suffix before .jsonl for the Osprey domain files.",
    )
    benchmark_group.add_argument(
        "--max-tokens",
        type=int,
        default=None,
        help=(
            "Override max_tokens for every benchmark. Unset uses each "
            "benchmark's own default. The paper uses 4096."
        ),
    )
    benchmark_group.add_argument(
...
def main():
    args = parse_args()
    if args.domain_eval_data_dir:
        os.environ["BENCH_EVAL_DATA_DIR"] = args.domain_eval_data_dir
    os.environ["BENCH_EVAL_SUFFIX"] = args.domain_eval_suffix
    ...
    csv_file = os.path.join(
        args.output_dir,
        f"{args.name + '_' if args.name else ''}summary_{timestamp}.csv",
    )
    write_csv_summary(results, csv_file)
...
def write_csv_summary(results: dict, csv_file: str) -> None:
    """Flatten the nested result JSON into one row per (benchmark, config).

    The JSON keeps the per-request lists; this is the table you read.
    """
    rows = []
    for benchmark_name, runs in results.items():
        ...
        for run in runs:
            for metric in run.get("metrics", []):
                row = {key: metric.get(key) for key in CSV_FIELDS if key in metric}
                row["benchmark"] = benchmark_name
                for key in ("batch_size", "steps", "topk", "num_draft_tokens"):
                    row[key] = run.get(key)
                rows.append(row)
    if not rows:
        return
