...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config, layer_idx: int = 0):
        ...
        # Qwen3 supports an attention bias config flag (Llama-3 does not).
        ...
        # Qwen3-specific: per-head RMSNorm applied to Q and K AFTER projection
        # and reshape, BEFORE rotary embedding. Llama does not have these.
    ...
    def _init_rope(self):
        ...
        Osprey drafters inherit `rope_theta` and `max_position_embeddings` from
        the source backbone and set `rope_scaling: null`, so plain RoPE is the
        only variant this release needs. `default` and `llama3` scaling are
        also handled because both reduce to Qwen3RotaryEmbedding; any other
        scaling type is rejected rather than silently ignored.
        """
        ...
        if scaling_type == "default":
            ...
            raise ValueError(
                f"Unsupported RoPE scaling type {scaling_type!r} for the draft "
                "model. This release only implements plain RoPE."
            )
    ...
    def forward(
        ...
        # Qwen3-specific: per-head RMSNorm on Q and K BEFORE rotary embedding.
...
class Qwen3FlexAttention(Qwen3Attention):
    """
    Attention layer implemented with flex attention. We keep the parameters consistent with Qwen3Attention.
    ...
    def forward(
        ...
        # Shrink the attention mask to match the right-hand padding that each
        # TTT step appends.
...
class Qwen3FlashAttention(Qwen3Attention):
    """
    Attention layer implemented with flash attention. We keep the parameters consistent with Qwen3Attention.
    The used parameters are:
        - hidden_states: input hidden states
        - position_ids: position ids
    ...
    def forward(
        ...
        # Qwen3-specific: per-head RMSNorm on Q and K BEFORE rotary embedding.
        # Shape is (bsz, q_len, num_heads, head_dim); RMSNorm on head_dim
        # (last dim) is invariant to the (q_len, num_heads) layout.
        query_states = self.q_norm(query_states)
        key_states = self.k_norm(key_states)
...
class Qwen3DecoderLayer(nn.Module):
    def __init__(self, config, attention_backend: str = "sdpa", layer_idx: int = 0):
        ...
        if attention_backend == "sdpa":
            ...
            self.self_attn = Qwen3FlashAttention(config=config, layer_idx=layer_idx)
        else:
...
class Qwen3ForCausalLMEagle3(Eagle3DraftModel):
    ...
    def __init__(self, config, quant_config=None, attention_backend="sdpa") -> None:
        ...
        if self.num_hidden_layers == 1:
            self.midlayer = Qwen3DecoderLayer(
            ...
            self.layers = nn.ModuleList(
                [
                    Qwen3DecoderLayer(
                        config, attention_backend=attention_backend, layer_idx=idx
                    )
    ...
    def forward(
        ...
        hidden_states = self._run_backbone_layers(
    ...
    def project_hidden_states(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        assert hidden_states.size(-1) == target_hidden_size * 3
    ...
    def _uses_layer_stack(self) -> bool:
        return hasattr(self, "layers")

    def _prepare_layer_cache_hidden(self, cache_hidden: torch.Tensor):
        if cache_hidden is None or not self._uses_layer_stack():
            return cache_hidden
        is_legacy_single_layer_cache = (
            ...
            and (
                ...
                or (
                    len(cache_hidden[0]) > 0
                    and isinstance(cache_hidden[0][0], torch.Tensor)
                )
            )
        )
        if is_legacy_single_layer_cache:
            cache_hidden[:] = [[[], []] for _ in range(len(self.layers))]
        return cache_hidden
    ...
    def _run_backbone_layers(
        ...
        cache_hidden = self._prepare_layer_cache_hidden(cache_hidden)
        for idx, layer in enumerate(self.layers):
            layer_cache_hidden = None if cache_hidden is None else cache_hidden[idx]
            hidden_states = layer(
                input_emb=input_embeds,
                hidden_states=hidden_states,
    ...
    def backbone(
        ...
        return self._run_backbone_layers(
