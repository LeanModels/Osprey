...
def _calculate_settings(n):
    ...
    # The forward / backward kernels both iterate over the vocab dim in
    # BLOCK_SIZE chunks (`for i in range(0, n_cols, BLOCK_SIZE)`), so a
    # vocab larger than MAX_FUSED_SIZE is fine — we just cap the per-step
    # block to the supported max instead of refusing to launch. Without
    # this cap, vocabs > 131072 (e.g. Qwen3 = 151936) trigger a hard
    # RuntimeError at the first training step.
